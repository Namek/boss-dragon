 ...
 * <p>
 * This implementation is not synchronised so if you need multi-threaded access then consider
 * wrapping it using the {@link Collections#synchronizedList} method.
 * <p>
 * The iterators this list provides are <i>fail-fast</i>, so any structural modification, other than
 * through the iterator itself, cause it to throw a {@link ConcurrentModificationException}.
 * 
 * @author Mark Rhodes
 * @version 1.3
 * @see List
 ...
 * @see AbstractList
 * @param <T>
 *            the type of element that this sorted list will store.
 */
public class SortedList<T> extends AbstractList<T> implements Serializable {
	 ...
	 * 
	 * @param comparator
	 *            the {@code Comparator} to sort the elements by.
	 */
	 ...
	 * <p>
	 * This method only allows non-<code>null</code> values to be added, if the given object is
	 * <code>null</code>, the list remains unaltered and false returned.
	 * 
	 * @param object
	 *            the object to add.
	 ...
	 * <p>
	 * This method can be overridden by a subclass in order to change the definition of the
	 * {@code Node}s that this List will store.
	 * <p>
	 * This implementation uses the {@code Node#compareTo(Node)} method in order to ascertain where
	 * the given {@code Node} should be stored. It also increments the modCount for this list.
	 * 
	 * @param toAdd
	...
	protected void add(Node toAdd) {
		...
		modCount++; // see AbstractList#modCount, incrementing this allows for iterators to be
					// fail-fast..
	}
	 ...
	 * 
	 * @param other
	 *            the {@code SortedList} to compare this to.
	...
	/**
	 * Provides an {@code Iterator} which provides the elements of this {@code SortedList} in the
	 * order determined by the given {@code Comparator}.
	 * <p>
	 * This implementation allows the entire list to be iterated over in time <i>O(n)</i>, where
	 * <i>n</i> is the number of elements in the list.
	 * 
	 * @return an {@code Iterator} which provides the elements of this {@code SortedList} in the
	 *         order determined by the given {@code Comparator}.
	 */
	@Override
	...
	/**
	 * Returns the root node of this {@code SortedList}, which is <code>null</code> in the case that
	 * this list is empty.
	 * 
	 * @return the root node of this {@code SortedList}, which is <code>null</code> in the case that
	 *         this list is empty.
	 */
	 ...
	 * under the assumption that the given <code>obj</code> must have type <code>T</code> to be
	 * equal to the elements in this {@code SortedList}. Works in time
	 ...
	 *             in the case that the index is not a valid index.
	 */
	@Override
	 ...
	 * 
	 * @param value
	 *            the object to remove from this {@code SortedList}.
	 * @return <code>true</code> if the given object was found in this {@code SortedList} and
	 ...
	 * 
	 * @param toRemove
	 *            the {@code Node}, which must be a {@code Node} in this {@code SortedList}.
	 */
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             in the case that the the index is not valid.
	 */
	protected Node findNodeAtIndex(int index) {
		...
		int totalSmallerElements = current.leftChild == null ? 0 : current.leftChild
				.sizeOfSubTree();
		while (current != null) { // should always break, due to constraint above..
			...
			if (totalSmallerElements > index) { // go left..
				...
				totalSmallerElements -= current.rightChild == null ? 0 : current.rightChild
						.sizeOfSubTree();
			}
			else { // go right..
				totalSmallerElements++;
				current = current.rightChild;
				totalSmallerElements += current.leftChild == null ? 0 : current.leftChild
						.sizeOfSubTree();
			}
		}
		return current;
	 ...
	 * elements in it.
	 * <p>
	 * If the length of the given array is larger than the size of this {@code SortedList} then, the
	 * elements in this list are put into the start of the given array and the rest of the array is
	 * left untouched.
	 * 
	 * @return an array representation of this list.
	 ...
	 * 
	 * @return the minimum of all balance factors for nodes in this tree, or 0 if this tree is
	 *         empty.
	 */
	 ...
	 * purpose other than for testing.
	 * 
	 * @return the maximum of all balance factors for nodes in this tree, or 0 if this tree is
	 *         empty.
	 */
	...
	// Implementation of the AVL tree rebalancing starting at the startNode and working up the
	// tree...
	 ...
	 * is aware of their children and parent nodes, the height of the subtree rooted at that point
	 * and
	...
	protected class Node implements Comparable<Node> {
		 ...
		 * 
		 * @param t
		 *            the value which this node will store.
		 */
		protected Node(T t) {
			this.value = t;
		...
		// Removes this node if it's a leaf node and updates the number of children and heights in
		// the tree..
		...
		/**
		 * Returns the grand parent {@code Node} of this {@code Node}, which may be
		 * <code>null</code>.
		 * 
		...
		/**
		 * Returns the parent {@code Node} of this node, which will be <code>null</code> in the case
		 * that this is the root {@code Node}.
		 * 
		 ...
		 *         this node and the given node, this number is returned, otherwise this node's id
		 *         minus
		...
		/**
		 * Gets the node that is the next smallest in the tree, which is <code>null</code> if this
		 * is the smallest valued node.
		 * 
		 * @return the node that is the next smallest in the tree, which is <code>null</code> if
		 *         this is the smallest valued node.
		 */
		...
		protected final void updateCachedValues() {
			...
			while (current != null) {
				if (current.isLeaf()) {
					...
					int rightTreeHeight = current.rightChild == null ? 0
							: current.rightChild.height;
					current.height = 1 + Math.max(leftTreeHeight, rightTreeHeight);

					// deal with the number of children..
					int leftTreeSize = current.leftChild == null ? 0 : current.leftChild
							.sizeOfSubTree();
					int rightTreeSize = current.rightChild == null ? 0 : current.rightChild
							.sizeOfSubTree();
		 ...
		 * <p>
		 * This method is called every time the list is altered. It is first called on the deepest
		 * node which is affected by a given change and is then subsequently called on ancestors of
		 * this node until it is called on the root node.
		 * <p>
		 * It is therefore only suitable for updating cached values in the case that they are
		 * non-global and do not rely on the parent node having the correct value when being
		 * calculated.
		 * <p>
