...
import net.bossdragon.component.Fireball;
...
public class TopDownEntityDebugSystem extends EntitySystem {
	ComponentMapper<Size> mDimensions;
	...
	final Vector2 tmpDir2 = new Vector2();
	...
	protected void processSystem() {
		...
		for (int i = 0, n = actives.size(); i < n; ++i) {
			...
			final Size dims = mDimensions.get(entityId);
			final Transform transform = mTransform.get(entityId);
			final Vector2 pos = transform.getCurrentPos();
			final Vector2 dir = transform.toDirection(tmpDir2);

			tmpPos2d.set(pos.x, pos.y).sub(min).scl(projection).add(padding, padding);
			...
			if (dims != null) {
				tmpSize2d.set(dims.getWidth(), dims.getHeight()).scl(projection);
			}
			...
			if ((renderingMode & (RENDERING_SMALL_CIRCLES|RENDERING_SCALED_CIRCLES)) != 0 || dims == null) {
				...
				float rotation = -MathUtils.atan2(dir.y, dir.x) * MathUtils.radiansToDegrees;
